...
if (!empty($_SESSION['searched_user_id'])) {
    ...
    if(!empty($_SESSION['felhasznalo_id'])){
        $sql="SELECT felhasznalo_id FROM admins WHERE felhasznalo_id='".$_SESSION['felhasznalo_id']."'";
        if(!$result = $conn->query($sql)) echo $conn->error;
        if($result->num_rows > 0){
             $sql="SELECT admins.felhasznalo_id FROM admins WHERE admins.felhasznalo_id  ='".$_SESSION['searched_user_id']."' and layer =1";
    ...
    echo"<div id='flex-con' class='flex-container'>";
        if ($pictureIds) {
            ...
            foreach($pictureIds as $pictureId) {
                ...
                $sql = "SELECT picture_id FROM pictures WHERE felhasznalo_id = '".$_SESSION['searched_user_id']."'";
                if($result->num_rows > 0){
                    if($pictures->get_category()!="Profilkep"){
                        if ($pictures->get_felhasznalo_id()==$_SESSION['searched_user_id']) {
                            $i=0;
                $j=0;
                   $sql="SELECT COUNT(felhasznalo_id) FROM likes WHERE vote = 0 and liked_pic_id='".$pictureId."'";
                   if(!$result = $conn->query($sql)) echo $conn->error;
                   ...
                   if($result->num_rows > 0){
                    ...
                    $sql="SELECT  COUNT(felhasznalo_id) FROM likes WHERE vote = 1 and liked_pic_id='".$pictureId."'";
                   if(!$result = $conn->query($sql)) echo $conn->error;
                   if($result->num_rows > 0){
                    foreach($result->fetch_assoc() as $row){
                        $j=$row;
                            ...
                            if (!empty($_SESSION['felhasznalo_id'])) {
                                ...
                                '<form action="index.php?page=searchedUser&searched='.$_SESSION['search'].'" method="post" id="absolut">
                                    ...
                                    $sql="SELECT felhasznalo_id FROM favourite WHERE favorited_picture_id=".$pictureId." and felhasznalo_id=".$_SESSION['felhasznalo_id']." ";
                                    if(!$result = $conn->query($sql)) echo $conn->error;
                                    if($result->num_rows > 0){
